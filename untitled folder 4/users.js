...
app.get('/', function (request, response) {
    ...
    var query = 'SELECT * FROM users ORDER BY hscore DESC;';
...
app.get('/submit_score/:score', function (request, response) {
    console.log("user highscore: "+global.hscore_user)
    console.log("gamescore revieved: "+request.params.score)
    var s = request.params.score;
    var updateQuery = "UPDATE users SET hscore = '"+ global.hscore_user +"' WHERE username = '"+ global.logged_user +"';";
    if(s > global.hscore_user){
	updateQuery = "UPDATE users SET hscore = '"+ s +"' WHERE username = '"+ global.logged_user +"';";
	global.hscore_user = s;
	}
       db.none(updateQuery)
           .then(function (result) {
 	   response.redirect('/users')
